...
void main(void)
{
	vec3 lightPos = normalize(vec3(0.0f, 0.4f, 1.0f));
    ...
    vec3 L2 = vec3(wrlMatrix * vec4(lightPos, 0.0));
	// surface -> viewer
    ...
    vec3 color = (diff) * vec3(1.0, 1.0, 1.0);

	vec3 N_model = normalize(v_normal_model);

	float height = v_pos_model.y;
	float heightBlend = smoothstep(0.8, 1.8, height);
	vec4 maskros = texture(texMaskros, texCoord);
	vec4 dirt = texture(texDirt, texCoord);

	vec4 terrainColor = mix(maskros, dirt, heightBlend);

	outColor = vec4(
		vec3(terrainColor.x, terrainColor.y, terrainColor.z) * color,
		terrainColor.a);
}
