...
void main(void)
{
    ...
    vec3 color2 = (diff2 + spec2) * lightSourcesColorArr[2]; // blue along x
    ...
    vec3 color3 = (diff3 + spec3) * lightSourcesColorArr[3]; // white along z
	...
	vec3 color0 = (spec0 + diff0) * lightSourcesColorArr[0]; // red positional
	...
	vec3 color1 = (spec1 + diff1) * lightSourcesColorArr[1]; // green positional

	vec3 color = 0.4 * (color0 + color1 + color2 + color3);
